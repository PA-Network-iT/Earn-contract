...
/// @dev    Performs two idempotent actions from an address holding DEFAULT_ADMIN_ROLE on EarnCore:
///           1. Calls `EarnCore.setSubscriptionManager(manager)` which activates the subscription gate.
///           2. Optionally mints a genesis subscription to the admin address so it can act as a
