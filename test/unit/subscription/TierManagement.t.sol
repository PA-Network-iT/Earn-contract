...
import {InvalidTier, InvalidPrice, ZeroAddress} from "test/shared/subscription/SubscriptionErrors.sol";
...
contract TierManagementTest is SubscriptionTestBase {
    ...
    function test_addTierMonotonicallyIncrementsId() public {
        ...
        emit TierAdded(1, 200e6, 5);

        vm.prank(admin);
        uint16 tier1 = manager.addTier(200e6, 5, "ipfs://1");

        vm.prank(admin);
        uint16 tier2 = manager.addTier(500e6, 15, "ipfs://2");
    ...
    function test_addTierRevertsOnZeroPrice() public {
        ...
        manager.addTier(0, 1, "ipfs://x");
    }

    function test_setTierUpdatesFields() public {
        vm.prank(admin);
        uint16 tierId = manager.addTier(200e6, 5, "ipfs://old");

        vm.expectEmit(true, false, false, true, address(manager));
        emit TierUpdated(tierId, 250e6, 7, true);
        vm.prank(admin);
        manager.setTier(tierId, 250e6, 7, true, "ipfs://new");
    ...
    function test_setTierRevertsOnUnknownId() public {
        ...
        manager.setTier(42, 250e6, 7, true, "ipfs://x");
    }

    function test_removeTierMarksInactiveButKeepsRecord() public {
        vm.prank(admin);
        uint16 tierId = manager.addTier(200e6, 5, "ipfs://x");
    ...
    function test_allActiveTierIdsReflectsLifecycle() public {
        vm.startPrank(admin);
        uint16 t1 = manager.addTier(100e6, 1, "uri1");
        uint16 t2 = manager.addTier(200e6, 5, "uri2");
        uint16 t3 = manager.addTier(500e6, 15, "uri3");
        vm.stopPrank();
    ...
    function test_nonParameterManagerCannotAddTier() public {
        ...
        manager.addTier(200e6, 5, "ipfs://x");
    }
}
